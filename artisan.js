    ...
    function addNewProduct(productName, productPrice, productDescription, productCategory, productImageFile) {
        ...
        reader.onload = function(e) {
            ...
            // Add to main products list (for categories page) - THIS IS THE KEY CHANGE
            addProductToCategoriesPage(newProduct);
    ...
    function updateProduct(productName, productPrice, productDescription, productCategory, productImageFile) {
        ...
        function completeProductUpdate() {
            ...
            updateProductInCategoriesPage(artisanProductsData[productIndex]);
    ...
    function deleteProduct(productId) {
        if (confirm('Are you sure you want to delete this product?')) {
            ...
            // Also remove from categories page
            removeProductFromCategoriesPage(productId);
    ...
    // Add product to categories page - THE KEY FUNCTION
    function addProductToCategoriesPage(product) {
        // Get existing products from localStorage
        const existingProducts = JSON.parse(localStorage.getItem('egytopiaProducts') || '[]');
        ...
        if (existingIndex === -1) {
            ...
            localStorage.setItem('egytopiaProducts', JSON.stringify(existingProducts));
            console.log('Product added to categories page:', product);
        }
    }

    // Update product in categories page
    function updateProductInCategoriesPage(updatedProduct) {
        const existingProducts = JSON.parse(localStorage.getItem('egytopiaProducts') || '[]');
        ...
        if (productIndex !== -1) {
            ...
            localStorage.setItem('egytopiaProducts', JSON.stringify(existingProducts));
            console.log('Product updated in categories page:', updatedProduct);
        }
    }

    // Remove product from categories page
    function removeProductFromCategoriesPage(productId) {
        const existingProducts = JSON.parse(localStorage.getItem('egytopiaProducts') || '[]');
        const updatedProducts = existingProducts.filter(product => product.id !== productId);
        localStorage.setItem('egytopiaProducts', JSON.stringify(updatedProducts));
        console.log('Product removed from categories page:', productId);
    }
